# ⚡ Skill Estrategia de Contenido
![Skill](https://img.shields.io/badge/Claude-Skill%20Activa-7c3aed)
![Status](https://img.shields.io/badge/Estado-Producción-0f071e)
![Version](https://img.shields.io/badge/Version-1.0-black)
![Autor](https://img.shields.io/badge/Jonatan%20Correa-%E2%9A%A1-7c3aed)

Skill de Claude para generar estrategias de contenido profesionales, accionables y listas para ejecutar.

Diseñada para creadores, marcas, coaches, agencias y negocios que quieren dejar de improvisar su contenido.

---

## 🧠 ¿Qué hace esta skill?

Convierte ideas sueltas en una estrategia de contenido completa:

- Ideas de contenido
- Hooks optimizados
- Pilares de contenido
- Formatos por red social
- Calendario listo para ejecutar
- CTAs estratégicos

Todo estructurado y listo para ejecutar en minutos.

---

## ✳️ Instalación (1 minuto)

1. Abre Claude
2. Ve a **Personalizar**
3. Selecciona **Crear Skill**
4. Copia y pega el contenido del archivo `estrategia-contenido.md` en los campos correspondientes:
   - Nombre
   - Descripción
   - Instrucciones
5. Guarda la skill
6. Pidele una estrategias de contenido y comenzará automáticamente

---

## 📥 Descargar Skill

Accede al archivo listo para instalar directamente en Claude:

👉 [📥 Descargar .MD](https://raw.githubusercontent.com/jonatancorrea-ai/skill-estrategia-contenido/main/estrategia-contenido.md)

---

## 💡 Ideal para:

- Marcas personales
- Coaches y consultores
- Agencias de marketing
- Creadores de contenido
- Dueños de negocios

---

## 🚫 Evita estos errores comunes

- Pedir contenido sin definir audiencia
- Querer demasiadas ideas sin enfoque
- No tener un objetivo claro (venta, autoridad, crecimiento)

---

## 🚀 Comunidad MODO FUTURO ⚡

Aprende IA sin complicaciones y a tu ritmo

Accede a:
- herramientas
- automatizaciones
- estrategias reales
- recursos exclusivos

👉 Únete aquí: https://instagram.com/soyjonatancorrea

---

## ⚡ Consultoría SMART MEET

Si quieres implementar IA en tu marca o negocio:

→ Escríbeme desde la comunidad
